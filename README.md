...
site/
  ...
  fetch-vendor.command  re-downloads vendor/ (needed once, or after a version bump)
  assets/        images, video, Lottie wordmark, SVG (generated by the script)
  vendor/        Inter and the Lottie player, served from here rather than a CDN
...
The pages ask for no third-party resource at all: Inter and the Lottie player
live in `vendor/`, not on Google Fonts and jsdelivr. That matters because
visitors on networks where those hosts are unreachable would otherwise get the
...
and takes the SVG-only Lottie build (`lottie_light`), which is right because
neither wordmark file uses expressions, images or text layers.

**One external dependency is left**: the AirTrip walkthrough and both payment-split
recordings stream from Dropbox, as the design had them. See "Things to know".
